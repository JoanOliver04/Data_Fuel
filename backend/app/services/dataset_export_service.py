"""Export price_history + stations from SQLite to a training CSV.
...
(first 7 days) are dropped. The export is deterministic: rows are ordered by
``recorded_at`` and column order is fixed, so the same DB state always produces
...
DEFAULT_OUTPUT_PATH = _BACKEND_DIR / "data" / "datos.csv"
...
_LOW_COST_BRANDS: frozenset[str] = frozenset(
    {"PLENOIL", "PETROPRIX", "BALLENOIL", "GASAUTO", "FAMILY", "EASYGAS", "CHEAP"}
)

# Spanish road / highway patterns (substring match on brand + address, upper).
_HIGHWAY_PATTERNS: tuple[str, ...] = ("A-", "AP-", "N-", "CV-", "CARRETERA", "CTRA", "KM")
...
def _geodesic_km(lat: float, lon: float) -> float:
    return round(float(geodesic(_ALZIRA, (lat, lon)).km), 4)
...
def count_data_rows(path: Path) -> int:
    """Count data rows (excluding the header) in an exported CSV."""
...
def _validate_frame(df: pd.DataFrame) -> None:
    """Guard the produced frame: exact schema and a non-null, non-empty target."""
    if list(df.columns) != COLUMN_ORDER:
        raise ValueError(f"Export schema mismatch: {list(df.columns)} != {COLUMN_ORDER}")
    if df.empty:
        raise ValueError("Export produced zero rows after feature engineering")
    if df[TARGET_COLUMN].isna().any():
        raise ValueError(f"Export target column {TARGET_COLUMN!r} contains nulls")
...
async def export_dataset(
    ...
    """Export price history to a training CSV. Pass a custom factory in tests."""
    if factory is None:
    ...
    agg["_rolling_30"] = agg.groupby(["_station_id", "tipo_combustible"])["precio"].transform(
        lambda s: s.rolling(window=30, min_periods=1).mean()
    )
    agg["tendencia_ultimos_30_dias"] = (agg["precio"] - agg["_rolling_30"]).round(6)
    df_trend = agg[["_station_id", "tipo_combustible", "fecha", "tendencia_ultimos_30_dias"]]
    ...
    df["precio_medio_municipio"] = (
        df.groupby(["fecha", "municipio", "tipo_combustible"])["precio"].transform("mean").round(6)
    )
